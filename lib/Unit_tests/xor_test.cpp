...
TestableXOR::TestableXOR(size_t buffer_size) {
...
bool TestableXOR::TestingPrimitive() {
  ...
  const char* hex_xor =
      "16b9d1d1fd957ed41d738603fe6d97aafd184a20f3f606ecb81d5597872cdff2";
...
float TestableXOR::TimingPrimitive(SerialLogWriter* logger, size_t samples,
  ...
  long total = 0;
  if (verbose_sampling) {
    PrintSimpleHeader(logger, "xor_256", samples);
  ...
  if (!verbose_sampling) {
    PrintSimpleAvg(logger, "xor_256", avg);
  }
  return avg;
}
