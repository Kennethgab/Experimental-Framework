...
TestableElliptic::TestableElliptic(const struct uECC_Curve_t *curve, size_t privkey_size, size_t pubkey_size, size_t secret_size) {
  this->curve        = curve;
...
bool TestableElliptic::TestingPrimitive() {
  ...
  uECC_shared_secret(permanent_public, ephemeral_private, shared_secret1, curve);
  uECC_shared_secret(ephemeral_public, permanent_private, shared_secret2, curve);
  return EqualBuffer(shared_secret2, shared_secret1, privkey_size);
}



float TestableElliptic::TimingPrimitive() {
  ...
  for (int i = 0; i < samples; i++) {
    ...
    uECC_make_key(ephemeral_public, ephemeral_private, curve);
    uECC_shared_secret(permanent_public, ephemeral_private, client_secret, curve);
    end            = micros();
